...
Bei Endava\footnote{Der Firma in welcher diese Arbeit geschrieben wurde.} werden
die Arbeitsstunden über eine interne Webapp (ein Oracle System) festgehalten.
Dieses glänzt nicht in Usability, es führt aber kein Weg dran vorbei. Weiterhin
wird diese Webapp als \textbf{Oracle} bezeichnet\footnote{Unter diesem Namen ist
es firmenintern bekannt.}.
...
Woche festgehalten. Dabei wird angegeben an welchen Wochtage wieviele Stunden
an welchen Projekten gearbeitet wurde. Wenn man eine 40 Stundenwoche an einem
einzigen Projekt arbeitet ist das trivial. Wenn man aber als Werkstudent an
mehreren Projekten mit schwankenden Stundenzahlen arbeitet wird es schnell
komplizierter. Und wenn dazu noch dokumentiert werden soll woran gearbeitet
wurde noch umso mehr. In der Praxis heißt das: die ‘Timecard’ in Oracle muss
...
subcommands.'' - \cite{oclif}

\smallskip

oclif bietet ein System um ohne viel Boilerplate Kommandos zu bauen,
dessen Flaggen und Argumente zu beschreiben und dann eigene Businesslogik
auszuführen (siehe Abbildung \ref{fig:oclif-list}.) Während oclif Kommando- und
...
Attraktivitätsumfrage nach \cite{attrakdiff} durchgeführt. Es wurde nach
...
Bei \cite{Westerman_1997} wurde das CLI bei Wahl des Interface deutlich weniger
...
\begin{figure}[H]
  ...
  \begin{minipage}[b]{0.4\textwidth}
    ...
    \caption{Mittelwerte über die von \cite{attrakdiff} definierten Kategorien.}
...
unterschiedlicher wird das Produkt bewertet.'' - \cite{attrakdiff} via
